...
- [ ] **§5 `ADMIN_TOKEN` provisioning (TIME-SENSITIVE unblock — do first):** the 3 function gates are live on main and Base44 redeploys them; until `ADMIN_TOKEN` exists as a Base44 env var, `importBodyData`/`importExerciseDb`/`selfTest` all return 503 (fails closed by design). Generate ≥32-byte secure random → set as Base44 env var `ADMIN_TOKEN` (same place `PROXY_SECRET`/`MCP_TRUSTED_KEY` live) → store a recovery copy in IdeaForge Secrets as `other/ANATOME_ADMIN_TOKEN` (raw token NOT committed to the mirror). Smoke test: no-auth → 401 + zero Exercise rows deleted; correct bearer → imports succeed.
- [x] **§4 API security headers — DONE on Cloudflare Worker (`api/src/index.ts:37-47`):** Hono middleware sets `X-Frame-Options: DENY`, `Permissions-Policy: geolocation=(), microphone=(), camera=()`, `X-Content-Type-Options: nosniff`, `Strict-Transport-Security`, `Referrer-Policy` on every response (incl. errors) via `await next()`. Locked in by `api/test/securityHeaders.test.ts` (3 tests, asserts on 200 + ≥400 + 404). Full suite 40/40.
- [ ] **§4 frontend (Base44-UI) security headers — PLATFORM LIMITATION, open decision:** the Anatome **UI is hosted on Base44**, which exposes **no app-level response-header control** → `X-Frame-Options`/`Permissions-Policy` cannot be set on the Base44-hosted UI origin. Decision needed: (a) accept the Base44-UI header gap as a known platform limitation and note it in the Launchpad re-submission, OR (b) front the Base44 UI with a Cloudflare proxy that injects the headers (bigger arch change). The API origin is now covered regardless.
- [ ] Re-run the Base44 security scan → confirm the three functions go **green**; the API-origin header findings should go green; the Base44-UI origin header finding may persist (platform limit) — document that in the re-submission if so.
...
- [x] **Bonus:** bake the same security-header rules into the Cloudflare Workers `/api` port — DONE (see §4 API above; the "future split" already has it).
